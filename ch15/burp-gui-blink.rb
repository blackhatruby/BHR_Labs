...
# Description:
#   Burp extension that is heavily depends on GUI components with "red blinking tab"!
# Requirements:
...
class BurpExtender
  ...
  def registerExtenderCallbacks(callbacks)
    @callbacks     = callbacks
    extension_name = 'BlackHat Ruby with blinking tab'
    @callbacks.setExtensionName("Blinking Tab")

    @itab = JTabbedPane.new
    @itab_caption = 'Blinking iTab..'
    ...
    5.times {blink_tab(@itab, Color::RED, 1) ; sleep(0.5)}
    # blink_tab(@itab, Color::ORANGE, 2)
    # blink_tab(@itab, Color::BLUE, 2)

    label_title = '<html><h1>' + "Black Hat 💎" + '</h1></html>'
  ...
  private
  # makes a given tab to blink for spacified perioid of times.
  #
  # @param [ITab] tab
  # @param [Color] color
  # @param [Integer] delay the blinking
  #
  # @return issues
  def blink_tab(tab, color, delay = 2)
    sleep 2 if getTabbedPane(tab).nil? # sleep 2 seconds if the tab isn't yet loaded
    tab_obj   = getTabbedPane(tab)
    tab_index = tab_obj.indexOfComponent(tab)
    tab_obj.setForegroundAt(tab_index, color)
    tab_obj.setBackgroundAt(tab_index, color)
    sleep delay
    tab_obj.setForegroundAt(tab_index, nil)
    tab_obj.setBackgroundAt(tab_index, nil)
  end

  def getTabbedPane(tab)
    tab.getParent
  end
